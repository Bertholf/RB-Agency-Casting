	...
	add_action('wp_head', 'rb_agency_casting_head');
		function rb_agency_casting_head(){
			if( !is_admin() ) {
				...
				echo "<link rel=\"stylesheet\" href=\"". RBAGENCY_PLUGIN_URL ."assets/css/buttons-min.css\" type=\"text/css\" media=\"screen\" />\n";
			}
		}
...
//for admin
			...
			$output .= "<ul>";
				...
				if(count($job_types) > 0 ){
					foreach($job_types as $jobtype){
						...
						//$output .= "        <a href=\"".$current_url."/?jobtype=".$jobtype->Job_Type_ID."\">".$jobtype->Job_Type_Title."</a>";
						$output .= "    </li>\n";
					}
				}
			$output .= "</ul>";
				...
				foreach($results as $job){
					...
					if($rb_agency_options_arr["rb_agency_option_hide_apply_this_job_button"]==0){
						if(is_user_logged_in()){
							...
							$output .= "<a href=\"".get_bloginfo("wpurl")."/profile-login\" id='apply_job_btn'>Apply to this Job</a>";
						}
					}
	    ...
	    function job_list_shortcode($atts){
			...
			#Get job type ids
			if(!empty($a['show_job_type'])){
				$job_type_id_arr = [];
				$job_type_arr = explode(",",$a['show_job_type']);
				
				foreach($job_type_arr as $job_type){
					$sql = "SELECT Job_Type_ID FROM ".table_agency_casting_job_type." WHERE Job_Type_Title = '".ucfirst($job_type)."'";
					$job_type_data = $wpdb->get_row($sql,ARRAY_A);
					$job_type_id_arr[] = $job_type_data["Job_Type_ID"];
				}
			}

			if($a['show_category_button'] == "true"){
				$output .= "<div id=\"job-types\">";
				$output .= "<ul>";
			...
			$where_job_type_id_arr = "";
			if(!empty($job_type_id_arr)){
				foreach($job_type_id_arr as $ids){
					$where_job_type_id_arr[] = " FIND_IN_SET($ids,jobs.Job_Type) >0 ";
				}
			}
			$where_job_type_id = implode(" OR ",$where_job_type_id_arr);
			$where_job_type_id = !empty($where_job_type_id) ? " AND ".$where_job_type_id : "";
			$query = "SELECT jobs.*, agency.* FROM ". table_agency_casting_job . " jobs "
					." INNER JOIN ".table_agency_casting." agency "
					." ON agency.CastingUserLinked = jobs.Job_UserLinked "
					." WHERE jobs.Job_ID > 0  AND (jobs.Job_Visibility = 1 OR jobs.Job_Visibility = 2) ".$where_job_type_id;
	    ...
	    add_shortcode( 'job-list', 'job_list_shortcode' );
	    ...
	    function rb_agency_detail_profile_castingjob($JobID){
	    	...
	    	foreach($custom_fields as $custom_field){
	    		if($custom_field["ProfileCustomView"] == 0 || $custom_field["ProfileCustomView"] == 1){
		   				rb_agency_detail_profile_castingjob_func($custom_field,$JobID);
		   		}else{
		   			if($userLevel == 0){
	   				}else{
	   					rb_agency_detail_profile_castingjob_func($custom_field,$JobID);
	    ...
	    function rb_agency_detail_profile_castingjob_func($result,$JobID){
	    	$ProfileCustomID = $result['ProfileCustomID'];
		       $ProfileCustomTitle = $result['ProfileCustomTitle'];
			   $ProfileCustomType  = $result['ProfileCustomType'];
			   $ProfileCustomOptions = $result['ProfileCustomOptions'];
